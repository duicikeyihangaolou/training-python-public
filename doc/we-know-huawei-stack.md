...
```bash
# 第一次：全量构建
docker build -t lab-demo:cache-test .

# 改 app/main.py 一行，再构建——应只重建 COPY app 之后的层
# 改 requirements.txt——应从 pip install 层重建
docker history lab-demo:cache-test --no-trunc | head
```
...
```bash
export SWR_REGISTRY=swr.cn-north-4.myhuaweicloud.com   # Stack 现场换行内域名
export SWR_NAMESPACE=your-org
...
| 公有云自学                         | Stack 生产                |
| ---------------------------------- | ------------------------- |
| `swr.cn-north-4.myhuaweicloud.com` | 行内 registry FQDN        |
...
```bash
OBS_ENDPOINT=https://obs.cn-north-4.myhuaweicloud.com   # Stack：行内 endpoint
...
class ObjectStorage:
    ...
    def upload_bytes(self, key: str, data: bytes, content_type: str = "application/octet-stream") -> str:
        resp = self._client.putContent(
            self._bucket, key, data, contentType=content_type
        )
        if resp.status >= 300:
...
> - [esdk-obs-python GitHub](https://github.com/huaweicloud/huaweicloud-sdk-python-obs)
...
- 网络超时：**有限次重试** + 指数退避。
- **幂等**：同一 Key 覆盖是否允许？需与业务/运维 **书面约定**。

#### 2.3.5 成本意识

- OBS ≠ 无限磁盘；冷热分层、生命周期由平台策略承载；研发避免无限期堆积临时 Key。

#### Demo 7：预签名下载（讲授 + 代码片段）
...
3. 开通 **SWR**、**OBS**（按区域，建议固定一个 region，如 `cn-north-4`）。
...
pip install esdk-obs-python fastapi "uvicorn[standard]"
```
...
**预签名代码**：见 [2.3 Demo 7](#23-obs-工程化最佳实践)。
...
**验收**：浏览器或 curl 能经预签名 URL 下载；响应头 `Content-Type` 正确。
...
| 2  | SWR 推送 Demo 4               | 本讲义 1.5 + [SWR 用户指南](https://support.huaweicloud.com/usermanual-swr/swr_01_0001.html)        |
